...
const Tree = ({ navigation }) => {
  ...
  useEffect(() => {
    if (isFocused) {
      ...
      //getDownline()
      setIsLoading(true)
      Promise.all([getDownline(), getNetinfo()]).then(res => {
        setIsLoading(false)
      }).catch(e => {
        //console.log('4');
        setIsLoading(false)
  ...
  const getNetinfo = () => {
    const promise = new Promise((resolve, reject) => {
      axios.get(Config.API_NET_INFO + `?id=${userReducer.id}`,
        {
          headers: {
          ...
          setNetinfo(result.data)
          resolve(result.data);
          console.log('Netinfo ', result.data)
        }, (err) => {
          reject(err);
        })
    })
  ...
  const getDownline = (id = null, slot_x = null, slot_y = null, status = 1) => {
    //alert('id: ',id)  
    let slot_tree = ''
    setIsLoading(true)
    if (id == null) {
      id = userReducer.id
      slot_x = userReducer.slot_x
      slot_y = userReducer.slot_y
    }
    console.log('getDownline', id + ':' + slot_x + ':' + slot_y)

    const promise = new Promise((resolve, reject) => {
      //alert(id)
      axios.get(Config.API_SLOT_TREE_DETAIL + `?id=${id}&slot_x=${slot_x}&slot_y=${slot_y}&status=${status}`,
        {
          headers: {
            Authorization: `Bearer ${TOKEN}`,
            'Accept': 'application/json'
          ...
          <View style={{ alignItems: 'center' }}>
            ...
            <ScrollView horizontal>
              ...
              <View style={styles.scrollBox1}>
                {data['prev_x'] >= 0 &&
                  <View style={{ alignItems: 'center' }}>
                    <TouchableOpacity onPress={() => getDownline(userReducer.id, data['prev_x'], data['prev_y'])}>
                ...
                <View style={styles.boxdata3group1}>
                  <View style={{ flexDirection: 'row' }}>
                    <View style={styles.boxdata31}>
                      {data['slots'][2][0]['data'] != '' &&
                        <TouchableOpacity onPress={() => { getDownline(userReducer.id, data['slots'][2][0]['data'].slot_x, data['slots'][2][0]['data'].slot_y); }}>
                          <View style={styles.boxCircel}>
                    ...
                    <View style={styles.boxdata31}>
                      ...
                      {data['slots'][2][1]['data'] != '' &&
                        <TouchableOpacity onPress={() => { getDownline(userReducer.id, data['slots'][2][1]['data'].slot_x, data['slots'][2][1]['data'].slot_y); }}>
                          <View style={styles.boxCircel}>
                            <Icon name='arrow-down-outline' size={50} color={"rgb(0,0,0)"} />
                          </View>
                        </TouchableOpacity>
                    ...
                    <View style={[styles.boxdata31, { marginLeft: 8 }]}>
                      {data['slots'][2][2]['data'] != '' &&
                        <TouchableOpacity onPress={() => { getDownline(userReducer.id, data['slots'][2][2]['data'].slot_x, data['slots'][2][2]['data'].slot_y); }}>
                          <View style={styles.boxCircel}>
                            <Icon name='arrow-down-outline' size={50} color={"rgb(0,0,0)"} />
                          </View>
                        </TouchableOpacity>
                    ...
                    <View style={styles.boxdata31}>
                      {data['slots'][2][3]['data'] != '' &&
                        <TouchableOpacity onPress={() => { getDownline(userReducer.id, data['slots'][2][3]['data'].slot_x, data['slots'][2][3]['data'].slot_y); }}>
                          <View style={styles.boxCircel}>
                            <Icon name='arrow-down-outline' size={50} color={"rgb(0,0,0)"} />
                          </View>
                        </TouchableOpacity>
          ...
          <View style={styles.tableGroup1}>
            ...
            <View style={styles.tableGroup}>
              ...
              <View style={styles.col2}>
                <Text style={styles.colR}> {userReducer.activations.name.toUpperCase()} </Text>
              </View>
            </View>
            <View style={styles.tableGroup}>
              <View style={styles.col1}>
              ...
              <View style={styles.col2}>
                <Text style={styles.colR}> {netinfo.level_total} </Text>
              </View>
            </View>
            <View style={styles.tableGroup}>
              <View style={styles.col1}>
              ...
              <View style={styles.col2}>
                <Text style={styles.colR}> {netinfo.left_total} </Text>
              </View>
            </View>
            <View style={styles.tableGroup}>
              <View style={styles.col1}>
              ...
              <View style={styles.col2}>
                <Text style={styles.colR}> {netinfo.right_total} </Text>
              </View>
            </View>
            <View style={styles.tableGroup}>
              <View style={styles.col1}>
              ...
              <View style={styles.col2}>
                <Text style={styles.colR}> {netinfo.ref_total} </Text>
              </View>
            </View>
          </View>
